...
{
    public class PostgreSqlDataProvider : BaseDataProvider, IRsDataProvider
    {
        /// <summary>
        ...
        private string GetSequenceName<TEntity>(DataConnection dataConnection) where TEntity : BaseEntity
        {
            ...
            if (descriptor is null)
                throw new RsException($"Mapped entity descriptor is not found: {typeof(TEntity).Name}");
            ...
            if (string.IsNullOrEmpty(columnName))
                throw new RsException("A table's primary key does not have an identity constraint");
        ...
        public virtual string BuildConnectionString(IRsConnectionStringInfo nopConnectionString)
        {
            if (nopConnectionString is null)
                throw new ArgumentNullException(nameof(nopConnectionString));

            if (nopConnectionString.IntegratedSecurity)
                throw new RsException("Data provider supports connection only with login and password");
