...
namespace Rs.Server
{
    public partial class PluginsInfo:IPluginsInfo
    {
        ...
        protected readonly IRsFileProvider _fileProvider;
        ...
        protected virtual async Task<IList<string>> GetObsoleteInstalledPluginNamesAsync()
        {
            ...
            var filePath = _fileProvider.MapPath(RsPluginDefaults.InstalledPluginsFilePath);
            if (!_fileProvider.FileExists(filePath))
            {
                //if not, try to parse the file that was used in previous nopCommerce versions
                filePath = _fileProvider.MapPath(RsPluginDefaults.ObsoleteInstalledPluginsFilePath);
                if (!_fileProvider.FileExists(filePath))
            ...
            return JsonSerializer.Deserialize<IList<string>>(text);
        }
        ...
        {
            var pluginsInfo = JsonSerializer.Deserialize<PluginsInfo>(json);
        ...
        public PluginsInfo(IRsFileProvider fileProvider)
        {
