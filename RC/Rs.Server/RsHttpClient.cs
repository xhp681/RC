...
namespace Rs.Server
{
    public partial class RsHttpClient
    {
        ...
        public RsHttpClient(AdminAreaSettings adminAreaSettings,
            HttpClient client,
            ...
            IWorkContext workContext)
        {
            ...
            client.DefaultRequestHeaders.Add(HeaderNames.UserAgent, $"RsCommerce-{RsVersion.CURRENT_VERSION}");
        ...
        public virtual async Task<string> GetCopyrightWarningAsync()
        {
            ...
            var url = string.Format(RsCommonDefaults.NopCopyrightWarningPath,
        ...
        public virtual async Task<RssFeed> GetNewsRssAsync()
        {
            //prepare URL to request
            var language = _languageService.GetTwoLetterIsoLanguageName(await _workContext.GetWorkingLanguageAsync());
            var url = string.Format(RsCommonDefaults.NopNewsRssPath,
                RsVersion.CURRENT_VERSION,
                _webHelper.IsLocalRequest(_httpContextAccessor.HttpContext.Request),
        ...
        {
            //prepare URL to request
            var url = string.Format(RsCommonDefaults.NopInstallationCompletedPath,
                RsVersion.CURRENT_VERSION,
                _webHelper.IsLocalRequest(_httpContextAccessor.HttpContext.Request),
        ...
        public virtual async Task<string> GetExtensionsCategoriesAsync()
        {
            //prepare URL to request
            var language = _languageService.GetTwoLetterIsoLanguageName(await _workContext.GetWorkingLanguageAsync());
            var url = string.Format(RsCommonDefaults.NopExtensionsCategoriesPath, language).ToLowerInvariant();
        ...
        public virtual async Task<string> GetExtensionsVersionsAsync()
        {
            //prepare URL to request
            var language = _languageService.GetTwoLetterIsoLanguageName(await _workContext.GetWorkingLanguageAsync());
            var url = string.Format(RsCommonDefaults.NopExtensionsVersionsPath, language).ToLowerInvariant();
            ...
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            //prepare URL to request
            var language = _languageService.GetTwoLetterIsoLanguageName(await _workContext.GetWorkingLanguageAsync());
            var url = string.Format(RsCommonDefaults.NopExtensionsPath,
