...
namespace Rs.Server.Plugins
{
    public partial class PluginService : IPluginService
    {
        ...
        public virtual Task<string> GetPluginLogoUrlAsync(PluginDescriptor pluginDescriptor)
        {
            ...
            var logoExtension = RsPluginDefaults.SupportedLogoImageExtensions
                .FirstOrDefault(ext => _fileProvider.FileExists(_fileProvider.Combine(pluginDirectory, $"{RsPluginDefaults.LogoFileName}.{ext}")));
            if (string.IsNullOrWhiteSpace(logoExtension))
                return Task.FromResult<string>(null);

            var storeLocation = _webHelper.GetStoreLocation();
            var logoUrl = $"{storeLocation}{RsPluginDefaults.PathName}/" +
                $"{_fileProvider.GetDirectoryNameOnly(pluginDirectory)}/{RsPluginDefaults.LogoFileName}.{logoExtension}";
            ...
            if (checkDependencies)
            {
                ...
                if (descriptor.DependsOn?.Any() ?? false)
                {
                    ...
                    if (dependsOn.Any())
                    {
                        ...
                        throw new RsException(errorMessage);
                    }
                }
            }
