...
namespace Rs.Server.Plugins
{
    public class UploadService : IUploadService
    {
        ...
        public UploadService(IRsFileProvider fileProvider,
        ...
        protected virtual IList<UploadedItem> GetUploadedItems(string archivePath)
        {
            ...
            var uploadedItemsFileEntry = archive.Entries
                .FirstOrDefault(entry => entry.Name.Equals(RsPluginDefaults.UploadedItemsFileName, StringComparison.InvariantCultureIgnoreCase)
            ...
            return JsonSerializer.Deserialize<IList<UploadedItem>>(reader.ReadToEnd());
        }
        ...
        protected virtual IDescriptor UploadSingleItem(string archivePath)
        {
            ...
            var pluginsDirectory = _fileProvider.MapPath(RsPluginDefaults.Path);

            //get path to the themes directory
            var themesDirectory = string.Empty;
            if (!string.IsNullOrEmpty(RsPluginDefaults.ThemesPath))
                themesDirectory = _fileProvider.MapPath(RsPluginDefaults.ThemesPath);
            ...
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                ...
                if (rootDirectories.Count != 1)
                {
                    throw new Exception("The archive should contain only one root plugin or theme directory. " +
                        ...
                        $"To upload multiple items, the archive should have the '{RsPluginDefaults.UploadedItemsFileName}' file in the root");
                }
                ...
                foreach (var entry in archive.Entries)
                {
                    ...
                    var isPluginDescriptor = entry.FullName
                        .Equals($"{uploadedItemDirectoryName}/{RsPluginDefaults.DescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);

                    //or whether it's a theme descriptor
                    var isThemeDescriptor = entry.FullName
                        .Equals($"{uploadedItemDirectoryName}/{RsPluginDefaults.ThemeDescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);
                    ...
                    if (isPluginDescriptor)
                    {
                        ...
                        if (!((PluginDescriptor)descriptor).SupportedVersions.Contains(RsVersion.CURRENT_VERSION))
                            throw new Exception($"This plugin doesn't support the current version - {RsVersion.CURRENT_VERSION}");
                    }
        ...
        protected virtual IList<IDescriptor> UploadMultipleItems(string archivePath, IList<UploadedItem> uploadedItems)
        {
            ...
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                foreach (var item in uploadedItems)
                {
                    ...
                    if (!item.SupportedVersions?.Contains(RsVersion.CURRENT_VERSION) ?? true)
                        continue;
                    ...
                    if (item.Type == UploadedItemType.Plugin)
                        descriptorPath = $"{itemPath}{RsPluginDefaults.DescriptionFileName}";

                    if (item.Type == UploadedItemType.Theme && !string.IsNullOrEmpty(RsPluginDefaults.ThemeDescriptionFileName))
                        descriptorPath = $"{itemPath}{RsPluginDefaults.ThemeDescriptionFileName}";
                    ...
                    //ensure that the plugin current version is supported
                    if (descriptor is PluginDescriptor pluginDescriptor && !pluginDescriptor.SupportedVersions.Contains(RsVersion.CURRENT_VERSION))
                        continue;
        ...
        public virtual IList<IDescriptor> UploadPluginsAndThemes(IFormFile archivefile)
        {
            ...
            try
            {
                ...
                var tempDirectory = _fileProvider.MapPath(RsPluginDefaults.UploadsTempPath);
        ...
        public virtual void UploadIconsArchive(IFormFile archivefile)
        {
            ...
            try
            {
                ...
                var storeIconsPath = _fileProvider.GetAbsolutePath(string.Format(RsCommonDefaults.FaviconAndAppIconsPath, _storeContext.GetActiveStoreScopeConfigurationAsync().Result));
        ...
        public virtual void UploadFavicon(IFormFile favicon)
        {
            ...
            var storeFaviconPath = _fileProvider.GetAbsolutePath(string.Format(RsCommonDefaults.FaviconAndAppIconsPath, _storeContext.GetActiveStoreScopeConfigurationAsync().Result));
        ...
        public virtual void UploadLocalePattern(CultureInfo cultureInfo = null)
        {
            ...
            try
            {
                ...
                var ziplocalePatternPath = getPath(RsCommonDefaults.LocalePatternPath, RsCommonDefaults.LocalePatternArchiveName);
                if (!_fileProvider.GetFileExtension(ziplocalePatternPath)?.Equals(".zip", StringComparison.InvariantCultureIgnoreCase) ?? true)
                    throw new Exception($"Archive '{RsCommonDefaults.LocalePatternArchiveName}' to retrieve localization patterns not found.");
                ...
                if (!(checkDirectoryExists(RsCommonDefaults.LocalePatternPath, currentCulture.Name) ||
                    checkDirectoryExists(RsCommonDefaults.LocalePatternPath, currentCulture.TwoLetterISOLanguageName)))
                {
                    var cultureToUse = string.Empty;

                    //3. Unpack the archive into a temporary folder
                    ZipFile.ExtractToDirectory(ziplocalePatternPath, getPath(RsCommonDefaults.LocalePatternPath, tempFolder));

                    //4. Search in the temp unpacked archive a folder with locales by culture
                    var sourcelocalePath = _fileProvider.Combine(getPath(RsCommonDefaults.LocalePatternPath, tempFolder), currentCulture.Name);
                    if (_fileProvider.DirectoryExists(sourcelocalePath))
                    {
                        cultureToUse = currentCulture.Name;
                    }

                    var sourcelocaleISOPath = _fileProvider.Combine(getPath(RsCommonDefaults.LocalePatternPath, tempFolder), currentCulture.TwoLetterISOLanguageName);
                    ...
                    {
                        var destlocalePath = getPath(RsCommonDefaults.LocalePatternPath, cultureToUse);
            ...
            finally
            {
                ...
                if (checkDirectoryExists(RsCommonDefaults.LocalePatternPath, tempFolder))
                    _fileProvider.DeleteDirectory(getPath(RsCommonDefaults.LocalePatternPath, tempFolder));
            }
        }
        ...
        public class UploadedItem
        {
            ...
            /// </summary>
            [JsonPropertyName("SourceDirectoryPath")]
