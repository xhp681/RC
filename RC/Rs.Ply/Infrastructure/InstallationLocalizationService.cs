...
namespace Rs.Ply.Infrastructure
{
    public partial class InstallationLocalizationService:IInstallationLocalizationService
    {
        ...
        public string GetBrowserCulture()
        {
            ...
            return userLanguages.FirstOrDefault()?.Split(',').FirstOrDefault() ?? RsCommonDefaults.DefaultLanguageCulture;
        }
        ...
        public virtual InstallationLanguage GetCurrentLanguage()
        {
            ...
            //try to get cookie
            var cookieName = $"{RsCookieDefaults.Prefix}{RsCookieDefaults.InstallationLanguageCookie}";
            ...
            foreach (var filePath in _fileProvider.EnumerateFiles(_fileProvider.MapPath("~/App_Data/Localization/Installation/"), "*.xml"))
            {
                ...
                foreach (XmlNode resNode in resources)
                {
                    ...
                    if (resNameAttribute == null)
                        throw new RsException("All installation resources must have an attribute Name=\"Value\".");
                    var resourceName = resNameAttribute.Value.Trim();
                    if (string.IsNullOrEmpty(resourceName))
                        throw new RsException("All installation resource attributes 'Name' must have a value.'");

                    if (resValueNode == null)
                        throw new RsException("All installation resources must have an element \"Value\".");
        ...
        public Dictionary<int, string> GetAvailableProviderTypes(int[] valuesToExclude = null, bool useLocalization = true)
        {
            return Enum.GetValues(typeof(DataProviderType))
                ...
                .ToDictionary(
                    ...
                    enumValue => useLocalization ? GetResource(enumValue.ToString()) : Utils.ConvertEnum(enumValue.ToString()));
        }
    }
}
