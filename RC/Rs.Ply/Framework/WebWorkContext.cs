...
namespace Rs.Ply.Framework
{
    public class WebWorkContext: IWorkContext
    {
        ...
        {
            var cookieName = $"{RsCookieDefaults.Prefix}{RsCookieDefaults.CustomerCookie}";
        ...
        public virtual async Task SetCurrentCustomerAsync(Customer customer = null)
        {
            if (customer == null)
            {
                ...
                if (_httpContextAccessor.HttpContext?.Request
                    ?.Path.Equals(new PathString($"/{RsTaskDefaults.ScheduleTaskPath}"), StringComparison.InvariantCultureIgnoreCase)
                    ?? true)
                {
                ...
                if (customer != null && !customer.Deleted && customer.Active && !customer.RequireReLogin)
                {
                    ...
                    var impersonatedCustomerId = await _genericAttributeService
                        .GetAttributeAsync<int?>(customer, RsCustomerDefaults.ImpersonatedCustomerIdAttribute);
        ...
        public virtual async Task SetWorkingLanguageAsync(Language language)
        {
            ...
            await _genericAttributeService.SaveAttributeAsync(customer, RsCustomerDefaults.LanguageIdAttribute, language?.Id ?? 0, store.Id);
        ...
        public virtual async Task<Language> GetWorkingLanguageAsync()
        {
            ...
            if (detectedLanguage == null && _localizationSettings.AutomaticallyDetectLanguage)
            {
                ...
                var alreadyDetected = await _genericAttributeService
                    .GetAttributeAsync<bool>(customer, RsCustomerDefaults.LanguageAutomaticallyDetectedAttribute, store.Id);
                ...
                if (!alreadyDetected)
                {
                    ...
                    if (detectedLanguage != null)
                    {
                        ...
                        await _genericAttributeService
                            .SaveAttributeAsync(customer, RsCustomerDefaults.LanguageAutomaticallyDetectedAttribute, true, store.Id);
                    }
                }
            }
            ...
            if (detectedLanguage != null)
            {
                ...
                var currentLanguageId = await _genericAttributeService
                    .GetAttributeAsync<int>(customer, RsCustomerDefaults.LanguageIdAttribute, store.Id);

                //save the detected language identifier if it differs from the current one
                if (detectedLanguage.Id != currentLanguageId)
                {
                    await _genericAttributeService
                        .SaveAttributeAsync(customer, RsCustomerDefaults.LanguageIdAttribute, detectedLanguage.Id, store.Id);
                }
            }
        ...
        public virtual async Task<Currency> GetWorkingCurrencyAsync()
        {
            ...
            var customerCurrencyId = await _genericAttributeService
                .GetAttributeAsync<int>(customer, RsCustomerDefaults.CurrencyIdAttribute, store.Id);
        ...
        public virtual async Task SetWorkingCurrencyAsync(Currency currency)
        {
            ...
            var customer = await GetCurrentCustomerAsync();
            var store = await _storeContext.GetCurrentStoreAsync();
            await _genericAttributeService.SaveAttributeAsync(customer, RsCustomerDefaults.CurrencyIdAttribute, currency?.Id ?? 0, store.Id);
        ...
        public virtual async Task<TaxDisplayType> GetTaxDisplayTypeAsync()
        {
            ...
            if (_taxSettings.AllowCustomersToSelectTaxDisplayType && customer != null)
            {
                ...
                var taxDisplayTypeId = await _genericAttributeService
                    .GetAttributeAsync<int?>(customer, RsCustomerDefaults.TaxDisplayTypeIdAttribute, store.Id);
        ...
        public virtual async Task SetTaxDisplayTypeAsync(TaxDisplayType taxDisplayType)
        {
            ...
            var store = await _storeContext.GetCurrentStoreAsync();
            await _genericAttributeService
                .SaveAttributeAsync(customer, RsCustomerDefaults.TaxDisplayTypeIdAttribute, (int)taxDisplayType, store.Id);
